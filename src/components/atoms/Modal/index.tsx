...
const Modal: React.FC<TProps> = ({ open, setOpen, children }) => {
    ...
    return open ? (
        <ContainerModal>
            <div
                className="overlay fixed top-0 left-0 w-full h-full z-30 cursor-pointer"
                onClick={() => setOpen(false)}
            />
            <motion.div
                initial={{ top: mobile ? "100%" : "60%" }}
                animate={{ top: mobile ? "25%" : "50%" }}
                transition={{ ease: "easeOut" }}
                className="modal sm:rounded-xl transform  sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 pt-0 sm:pt-4 p-4 z-40"
            >
                {mobile ? (
                    <MdOutlineMinimize
